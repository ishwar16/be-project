...
def lambda_handler(event, context):
    # Get the name of the image file from the event
    image_name = event['Records'][0]['s3']['object']['key']

    # Get the image file from S3
    response = s3.get_object(Bucket='my-bucket', Key=image_name)
...
def volumeCalculation():
    ...
    print("Volume = " + str(arr[0]) + " x " + str(arr[1]) + " x " + str(x))
    print((arr[0])*(arr[1])*(x))
